...
var_list={}
label_list={}
instruction_dict={"addf":"00000","subf":"00001","movf":"00010","add":"10000","sub":"10001","mov":"10011","ld":"10100","st":"10101","mul":"10110","div":"10111","rs":"11000","ls":"11001","xor":"11010","or":"11011","and":"11100","not":"11101","cmp":"11110","jmp":"11111","jlt":"01100","jgt":"01101","je":"01111","hlt":"01010"}
register={"R0":"000","R1":"001","R2":"010","R3":"011","R4":"100","R5":"101","R6":"110","FLAGS":"111"}
dict_A={"F_Addition":"00000","F_Subtraction":"00001","add":"10000","sub":"10001","mul":"10110","xor":"11010","or":"11011","and":"11100"}
    ...
    for i in range(len(read)):
        ...
        if (len(words))>0:
            if words[0]=='var' :
                if(var_at_start):
                    if len(words)!=2:
                            print("Error at line ",i+1," : variable definition incorrect ");     exit()
                    if  str(words[1][0]).isnumeric()==False and (words[1] in instruction_dict)==False and (words[1] in register)==False and (words[1] in label_list)==False:
                        if len(words)!=2:
                            ...
                            if(words[1] in register or words[1]=='FLAGS'):
                                print("Error at line ",i+1," : invalid variable name");      exit()
                            if(words[1] in var_list):
                                print("ERROR at line ",i+1," : Multiple declarations of variables ",words[1]); exit()
                            var_list[words[1]]=-1
                            variable_count+=1
                    else:
                        print("Error at line ",i+1," : invalid variable declaration ");     exit()
                else:
                    print("Error at line ",i+1," : invalid variable declaration ");      exit()
            else:
                var_at_start=0
    ...
    for i in range(len(read)):
        if len(read[i])>0:
            ...
            if(first_word[-1]==':' and len(first_word)==1):
                print("ERROR at line ",i+1," : invalid label definition (label name cant be null)"); exit()
            elif (first_word[-1]==':') and ((first_word[0:-1] in instruction_dict) or (first_word[0:-1] in register)):
                print("ERROR at line ",i+1," : keyword used as label"); exit()
            elif(first_word[-1]==':' and first_word[-2]==':'):
                print("ERROR at line ",i+1," : invalid label definition(label name cant be ':')"); exit()
            elif(''.join(first_word[0:-1:]) in register or ''.join(first_word[0:-1:])=='FLAGS'):
                print("ERROR at line ",i+1," : Invalid label name"); exit()
            elif(words[0][0:-1] in label_list):
                print("ERROR at line ",i+1," : label must be unique"); exit()
            elif(len(words)==1 and first_word[-1]==':'):
                print("ERROR at line ",i+1," : label statement empty"); exit()
            elif(first_word[-1]==':' and 'var' in words):
                print("ERROR at line ",i+1," : General Syntax Error"); exit()
            elif(first_word[-1]==':'):
...
def check_typeA(words,i):
    ...
    if len(words)!=4:
        print("ERROR at line ",i+1," : invalid syntax"); exit()
    elif 'FLAGS' in words:
        print("ERROR at line ",i+1," : invalid usage of FLAGS register"); exit()
    else:
        if ((words[1] not in regs) or (words[2] not in regs) or (words[3] not in regs)) :
            print("ERROR at line ",i+1,": invalid register"); exit()
    ...
    if(len(words)!=3):
        ...
        if words[0]=='mov':
            ...
            if(words[1] not in register):
                print("ERROR at line ",i+1," : invalid use of FLAGS register"); exit()
            elif(words[2]=='FLAGS'):
                print("ERROR at line ",i+1," : Inavlid use of FLAGS register"); exit()
            elif(words[2] not in register and words[2]!='FLAGS'):
                print("Error at line ",i+1," : SYNTAX ERROR"); exit()
...
def check_typeD(words,i):
    if(len(words)!=3):
        ...
        if(words[1] not in register):
            print("ERROR at line ",i+1," : not a valid register"); exit()
        elif(words[1]=='FLAGS'):
            print("ERROR at line ",i+1," : Inavlid use of FLAGS register"); exit()
        if(words[2] in label_list):
            print("ERROR at line ",i+1," : Misuse of variable as label"); exit()
        elif(words[2] not in var_list):
            print("ERROR at line ",i+1," : Variable "+"'"+ words[2]+"'"+" is not defined"); exit()      
    ...
    if(len(str)==2):
        for i in range(len(str)):
                if str[1] in label_list:
                    ...
                    print("ERROR at line ",j+1," : Misuse of variable as label"); exit(0)  
                else :
                    print("ERROR at line ",j+1," : Label is undefined"); exit(0)  
    else:
        print("Error at line ",j+1," : invalid instruction"); exit(0)
def counter():
    global read,read2
    counter=0
    for i in range(len(read)):
        if read[i]=="hlt":
            counter+=1
        if(counter>1):
            print("ERROR at line: ",i+1,"multiple hlt declarations"); exit()

def instructions():
    global read,read2
    if(read2[-1]) != 'hlt': 
        print("ERROR: hlt absent"); exit()
    ...
    for i in range(len(read)):
        ...
        if(len(read[i]))>0:
            if  (words[0]=='var' or (words[0] in main)):
                    ...
                    check_typeA(words,i)
                        ...
                        if ('.' in words[-1]):
                            if(float_bin1(words[-1][1::],5)==0):
                                print("ERROR at line ",i+1 ," : Illegal immediate value"); exit()
                ...
                print("ERROR at line ",i+1," : invalid instruction "); exit()
        ...
        if(len(read[i])>0):
            ...
            if(inst[0] not in label_list): 
                if(inst[0] in dict_A.keys()):
                    print(dict_A[inst[0]]+"00"+register[inst[1]]+register[inst[2]]+register[inst[3]])
                elif (inst[0] in dict_B.keys() and inst[-1][0]=='$' and '.' in inst[-1]):
                    print(dict_B[inst[0]]+register[inst[1]]+float_bin(inst[2][1::]))
